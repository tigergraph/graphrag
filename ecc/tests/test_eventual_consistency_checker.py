...
class TestEventualConsistencyChecker(unittest.TestCase):
    ...
    def test_fetch_and_process_vertex(
        ...
        mock_conn.runInstalledQuery.side_effect = [mock_response, "true"]

        # Construct and run the checker
        checker = EventualConsistencyChecker(
            6000,
            6000,
            graphname,
        ...
        mock_conn.runInstalledQuery.assert_any_call("Scan_For_Updates", {"v_type": "index1", "num_samples": 10})
        mock_conn.runInstalledQuery.assert_any_call(
