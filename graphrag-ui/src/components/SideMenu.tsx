  ...
  const deleteConversation = async (conversationId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Remove the deleted conversation from local state immediately
        ...
        // Refresh list from server in background to stay in sync
        fetchHistory2();
      }
    } catch {
      setDeletingConvoId(null);
                ...
                {conversations.map((item: any, idx: number) => {
                  ...
                  return (
                    <li key={`${item.conversation_id}-${idx}`} className="text-ellipsis">
                      <div className={`${isActive ? 'bg-gray-100 dark:bg-gray-800' : ''} rounded group`}>
                        <a 
                          href="#" 
