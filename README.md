...
| `hybrid_expand` | bool | `false` | When `true`, agent `hybrid_search` uses question expansion plus keyword/vector seeding (`GraphRAG_Hybrid_Search`) instead of vector-only hybrid search. Configure globally or per graph under `configs/graph_configs/<graph>/server_config.json`. |
| `hybrid_method` | string | `"similarity"` | Used when `hybrid_expand` is `true`. Options: `similarity` (vector seeds only), `keywords` (LLM keywords only), `both` / `all` (keyword seeds plus vector seeds). |
