...
class CommonTests:
    ...
    def setUpClass(cls, schema="all", use_wandb=True):
        ...
        def query_registration_test_generator(
            dataset, username, password, query_name, query_json
        ):
            # Need to extract q/a pairs before test is generated,
            # as otherwise we look at the last question/answer pair is used
        ...
        for suite in schemas:
            ...
            registration_tests = [
                query_registration_test_generator(
                    suite, config["username"], config["password"], q[0], q[1]
                )
                for q in prompts
            ]
            ...
            tests = list(
                questions.apply(
                    lambda x: question_test_generator(
                        suite, x, config["username"], config["password"]
                    ),
                    axis=1,
                )
            )
            ...
            for q_type in q_types:
                ...
                for dataset in unique_datasets:
                    ...
                    if final_df.shape[0] > 0:
                        cls.wandbLogger = wandb.init(
                            project="GraphRAG", config=cls.config
                        )
                        acc = (final_df["Answer Correct"].sum()) / final_df[
                            "Answer Correct"
                        ].shape[0]
                        not_wrong_perc = (
                            final_df["Answer Correct"].sum()
                            + (final_df["Answered Question"] == False).sum()
                        ) / final_df["Answer Correct"].shape[0]
                        ...
                        cls.wandbLogger.log(
                            {
                                ...
                                "Number of Questions": final_df["Answer Correct"].shape[
                                    0
                                ],
                            },
                            commit=True,
